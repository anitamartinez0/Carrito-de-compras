...
for (var i = 0; i < document.getElementsByClassName("producto").length; i++) {
    ...
    document.getElementsByClassName("producto")[i].onclick = function() {      //cuando se hace click en algun producto, se accede atraves del nombre de la clase!
        this.dataset.cantidad++;                                               //se aumenta la cantidad, con dataset obtengo el atributo desde html

        anadirCarrito(this.dataset.idproducto, this.dataset.img, this.dataset.des, this.dataset.precio, this.dataset.cantidad);  //se añade al carrito el producto con todos sus datos 
        precioTotal = precioTotal + carrito.precio;
        //Añadir al <p> con el id precioTotal la cantidad total de dinero
        document.getElementById("precioTotal").innerHTML = "Total: $" + precioTotal; //modifica el valor de precio total

        if (this.dataset.cantidad <= 1) {
            var compra = document.createElement("div");     //Agregar div con contenido unico de carrito de compras (solo puede existir 1)
            compra.setAttribute("id", carrito.id);          //se establece el valor del id en compra
...
function actualizador() {
    ...
    //Agregar Cantidad a carrito de compras  EL NUMERO DE CANTIDAD!!!
    ...
    //Agregar PALABRA Cantidad a carrito de compras PALABRA!!!
    compraDes = document.createElement("p");
    compraDes.setAttribute("class", "cantidad");
